...
install: ## Install dependencies
	@echo "Installing the 'lrytas' project..."
	...
	@echo "Installed the 'lrytas' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:alexandrainst/lrytas.git; \
	fi
...
publish-docs:  ## Publish documentation to GitHub Pages
	...
	@echo "Updated documentation website: https://alexandrainst.github.io/lrytas"

test:  ## Run tests
	@uv run pytest && uv run readme-cov

docker:  ## Build Docker image and run container
	@docker build -t lrytas .
	@docker run -it --rm lrytas
